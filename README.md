...
You'll find a file named `ExceptionInterface.php` in the `src/Account/Contract/Exception` directory with the following code:
...
Additionally, two base exceptions, `DomainException` and `RuntimeException` have been created in the `src/Account/Exception/`. I recommend using `DomainException` for exceptions generated by your code, and `RuntimeException` for exceptions generated by third party libraries and services.
